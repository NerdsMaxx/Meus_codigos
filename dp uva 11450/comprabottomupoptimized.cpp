...
int main()
{
	...
	for(int t=0;t<numdecasos;t++)
	{
		...
		for(int i=1;i<qtdpeca;i++)
		{
			...
			{
				if(podepagar[i1][j]==true)
				{
					for(int k=1;k<=preco[i][0];k++)
					{
						if((aux=j-preco[i][k])>=0)
						{podepagar[i2][aux]=true;}
					}
				}
			}
			swap(i1,i2);
			fill(podepagar[i2],podepagar[i2]+orcamento,false);
		}

		int i=0;
		for(;i<=orcamento&&podepagar[i1][i]!=true;i++);

		if(podepagar[i1][i]!=true){cout<<"no solution\n";}
